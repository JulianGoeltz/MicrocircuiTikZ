...
\newcommand{\pyramidalNeuron}[6]{
    ...
    \def\DesignLW{0.6mm * #5}
    \def\DesignLWIN{0.6mm * #5}
    \def\CoordPadding{0.05 * #5}

    \def\constApicalMainLength{0.4 * #5}
    \def\constApicalBrnY{\constApicalMainLength + 0.15 * #5}
    \def\constApicalBrnX{0.15 * #5}
    \def\constSomaHeight{0.38 * #5}
    \def\constSomaX{0.25 * #5}
    ...
    \def\constINheight{0.20 * #5}
    \def\constINwidth{0.10 * #5}
    \def\constINdendrite{0.10 * #5}

    % apical main + left
    \draw[color=clNeuronApical, line width=\DesignLW, line cap=round] (\x, \y - 0.2*\constApicalMainLength) -- (\x, \y+\constApicalMainLength) -- (\x -\constApicalBrnX, \y+\constApicalBrnY);
    % apical right
    \draw[color=clNeuronApical, line width=\DesignLW, line cap=round] (\x, \y+\constApicalMainLength) -- (\x +\constApicalBrnX, \y+\constApicalBrnY);
    % basal
    \draw[color=clNeuronBasal, line width=\DesignLW, line cap=round] (\x, \y-\constSomaHeight) -- (\x, \y-\constSomaHeight-\constBasalHeight);
    % soma
